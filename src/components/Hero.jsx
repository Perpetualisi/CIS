...
<text x="79" y="41" fill="${a}" font-size="26" font-family="'Sora',sans-serif" font-weight="800" text-anchor="middle">${val}</text>
...
/* NEW: floating mobile depth particles — adds 3D parallax dust to mobile hero */
const depthParticles=(a,count=14,seedOffset=0)=>{
  let g="";
  for(let i=0;i<count;i++){
    const seed=(i+seedOffset)*37.13;
    const x=(seed*53.7)%900;
    const y=40+((seed*91.3)%480);
    const r=1.4+((seed*17)%2.4);
    const dur=3.5+((seed*7)%4);
    const delay=((seed*3)%4);
    g+=`<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r.toFixed(1)}" fill="${a}" fill-opacity="0">
<animate attributeName="fill-opacity" values="0;0.5;0" dur="${dur.toFixed(2)}s" begin="${delay.toFixed(2)}s" repeatCount="indefinite"/>
<animateTransform attributeName="transform" type="translate" values="0 0;0 -22;0 0" dur="${(dur*1.4).toFixed(2)}s" begin="${delay.toFixed(2)}s" repeatCount="indefinite" calcMode="ease"/>
</circle>`;
  }
  return `<g>${g}</g>`;
};

/* NEW: rotating iso shard cluster — compact 3D accent that reads well at small sizes */
const isoShardCluster=(cx,cy,a,scale=1)=>{
  const s=scale;
  return `<g opacity="0">
<animate attributeName="opacity" from="0" to="1" dur="0.6s" begin="0.3s" fill="freeze"/>
<g transform="translate(${cx},${cy})">
<animateTransform attributeName="transform" type="rotate" additive="sum" from="0 0 0" to="360 0 0" dur="${18/s}s" repeatCount="indefinite"/>
${isoBox(0,0,34*s,34*s,24*s,T.chrome,a,a,0.85,a)}
${isoRing(0,0,52*s,52*s,38*s,0.6,a,0.3)}
${isoRing(0,0,70*s,70*s,50*s,0.4,a,0.14)}
</g>
</g>`;
};
...
<animate attributeName="stroke-opacity" values="0;0.55;0" dur="${1.2+i*.15}s" begin="${i*.18}s" repeatCount="indefinite"/>
  ...
  s+=scanVLine(a,7);
  if(mobile){ s+=depthParticles(a,16,2); s+=isoShardCluster(792,86,a,0.62); }
  else s+=badge(606,44,"99%","LIGHTHOUSE SCORE",a);
  s+=`</svg>`;
  return s;
};
...
<animate attributeName="stroke-dashoffset" from="0" to="-200" dur="2.8s" repeatCount="indefinite"/>
  ...
  if(mobile){ s+=depthParticles(a,16,9); s+=isoShardCluster(120,470,a,0.55); }
  else s+=badge(702,26,"10×","SEARCH ACCURACY","#22c55e");
  s+=`</svg>`;
  return s;
};
...
<line x1="${n.x}" y1="${n.y}" x2="450" y2="248" stroke="#ef4444" stroke-width="0.65" stroke-opacity="0.18" stroke-dasharray="5 4"/>
  ...
  s+=scanLine(a,4.5);
  if(mobile){ s+=depthParticles(a,16,4); s+=isoShardCluster(96,470,a,0.55); }
  else s+=badge(22,22,"0-DAY","THREAT RESPONSE",a);
  s+=`</svg>`;
  return s;
};
...
<animate attributeName="opacity" values="0.4;1;0.4" dur="${1+i*.12}s" begin="${i*.15}s" repeatCount="indefinite"/>
  ...
  if(mobile){ s+=depthParticles(a,16,7); }
  else s+=badge(716,396,"99.9%","UPTIME SLA",a);
  s+=`</svg>`;
  return s;
};
...
${orbitRing(mc.cx,mc.cy-20,230,72,a,14,2,0.1)}
  ...
  if(mobile){ s+=depthParticles(a,16,11); s+=isoShardCluster(800,470,a,0.55); }
  else s+=badge(22,454,"&lt;2HR","AVG RESPONSE",a);
  s+=`</svg>`;
  return s;
};
...
${n.main?`<text x="${n.x}" y="${n.y+4}" fill="#dde8f5" font-size="7" font-family="monospace" text-anchor="middle">CORE</text>`:""}
  ...
  if(mobile){ s+=depthParticles(a,16,13); }
  else s+=badge(658,26,"100G","MAX THROUGHPUT",a);
  s+=`</svg>`;
  return s;
};
...
<text x="768" y="37" fill="#ef4444" font-size="8.5" font-family="monospace" letter-spacing="0.8">&#9679; REC</text>
  ...
  if(mobile){ s+=depthParticles(a,16,5); }
  else s+=badge(22,450,"4K","AI RESOLUTION",a);
  s+=`</svg>`;
  return s;
};
...
<animate attributeName="stroke-dashoffset" from="0" to="-118" dur="2.2s" repeatCount="indefinite"/>
  ...
  if(mobile){ s+=depthParticles(a,16,6); }
  else s+=badge(22,22,"&lt;20ms","LATENCY",a);
  s+=`</svg>`;
  return s;
};
...
${braces(a)}`;
  ...
  s+=`<text x="${fv(cp.px)}" y="${fv(cp.py)}" fill="${a}" font-size="32" font-family="'Sora',sans-serif" font-weight="800" text-anchor="middle" letter-spacing="5" opacity="0">AV / MEDIA
...
${orbitRing(450,225,340,110,a,18,3,0.08)}
  ...
  if(mobile){ s+=depthParticles(a,16,8); }
  else s+=badge(326,468,"8K","DISPLAY QUALITY",a);
  s+=`</svg>`;
  return s;
};
  ...
  return (
    <section id="home" ref={heroRef}
      ...
      <style>{`
        ...
        .hD{font-family:'Sora',sans-serif!important;font-weight:800!important;letter-spacing:-0.01em;}
        ...
        @media(max-width:1023px){#home{margin-top:66px!important;height:80svh!important;min-height:460px!important;max-height:700px!important;}}
        ...
        @media(max-width:380px){#home{height:74svh!important;min-height:392px!important;}}
      ...
      <div className="lg:hidden hR absolute inset-0 overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div key={`m${current}`} className="absolute inset-0"
            ...
            <div className="absolute inset-0"><SVGScene sceneKey={sl.scene} accent={sl.accent} mobile/></div>
            <div className="absolute inset-0" style={{background:"linear-gradient(to bottom,rgba(5,12,24,.34) 0%,rgba(5,12,24,.08) 16%,rgba(5,12,24,.6) 46%,rgba(5,12,24,.95) 74%,#050c18 100%)"}}/>
            <div className="absolute inset-0" style={{background:"linear-gradient(to right,rgba(5,12,24,.5) 0%,transparent 60%)"}}/>
            <div className="absolute inset-x-0 bottom-0 z-10" style={{padding:"0 18px clamp(20px,5vw,30px)"}}>
              <div className="inline-flex items-center gap-2 mb-3 px-3 py-1.5 rounded-full" style={{background:`${sl.accent}24`,border:`1px solid ${sl.accent}55`}}>
                ...
                <span className="hM text-[9.5px] font-bold tracking-[.3em] uppercase" style={{color:sl.accent}}>{sl.tag}</span>
              </div>
              <h1 className="hD mb-2.5" style={{fontSize:"clamp(2.3rem,11.5vw,3.4rem)",lineHeight:.92,color:"#fff",textShadow:"0 2px 24px rgba(0,0,0,.45)"}}>{sl.headline}</h1>
              <div className="flex items-center gap-2.5 mb-2.5">
                <div className="flex-shrink-0 h-[2.5px] w-7 rounded-full" style={{background:sl.accent}}/>
                <p className="hB text-[10px] font-bold tracking-wide uppercase" style={{color:"rgba(255,255,255,.66)"}}>{sl.subheadline}</p>
              </div>
              <p className="hB leading-snug mb-4 font-medium" style={{color:"rgba(255,255,255,.74)",fontSize:".84rem",maxWidth:"38ch",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{sl.intro.replace(/\n/g," ")}</p>
              <div className="flex gap-2.5 mb-4">
                <a href="#contact" onClick={e=>scrollTo(e,"#contact")} className="bP hB flex-1 inline-flex items-center justify-center gap-1.5 py-3 px-3 text-[11px] font-extrabold tracking-wide uppercase rounded-sm" style={{background:sl.accent,color:"#fff"}}>{sl.cta.primary}<svg width="11" height="11" viewBox="0 0 12 12" fill="none"><path d="M2.5 6h7M6.5 3l3 3-3 3" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg></a>
                <a href="#services" onClick={e=>scrollTo(e,"#services")} className="bG hB inline-flex items-center justify-center py-3 px-3 text-[11px] font-bold tracking-wide uppercase whitespace-nowrap rounded-sm">{sl.cta.secondary}</a>
              </div>
              <div className="flex items-center gap-1.5">{SLIDES.map((_,i)=>(<button key={i} onClick={()=>manualNav(i)} className="h-[3px] rounded-full flex-shrink-0" style={{width:i===current?24:6,background:i===current?sl.accent:"rgba(255,255,255,.22)",transition:"all .32s"}}/>))}</div>
            </div>
          </motion.div>
        </AnimatePresence>
      </div>
            ...
            {SLIDES.map((s,i)=>(
              <button key={i} onClick={()=>manualNav(i)} className={`nI hB text-left pl-4 py-2.5 pr-2 ${i===current?"on":""}`} style={{"--ac":s.accent,borderLeftColor:i===current?s.accent:undefined}}>
                <div className="flex items-center justify-between">
                  <span className="text-[10.5px] font-semibold tracking-widest uppercase" style={{color:i===current?"#fff":"rgba(255,255,255,.3)",transition:"color .22s"}}>{s.tag}</span>
                  {i===current&&<span className="hM text-[8.5px]" style={{color:s.accent}}>{String(i+1).padStart(2,"0")}</span>}
                </div>
                {i===current&&<div className="hB text-[12px] font-bold mt-0.5" style={{color:"rgba(255,255,255,.82)"}}>{s.headline}</div>}
              </button>
            ))}
          </nav>
            ...
            <motion.div key={`ct${current}`}
              ...
              <div style={{maxWidth:580}}>
                <div className="hEnt d1 flex items-center gap-3 mb-5">
                  ...
                  <span className="hM text-[9.5px] font-bold tracking-[.46em] uppercase" style={{color:sl.accent}}>{sl.subheadline}</span>
                </div>
                <h1 className="hD hEnt d2 mb-4" style={{fontSize:"clamp(3.2rem,5.4vw,6.4rem)",lineHeight:.86,letterSpacing:"-0.01em",color:"#fff"}}>{sl.headline}</h1>
                <div className="hEnt d3 flex items-center gap-3 mb-4">
                  ...
                  <span className="hM text-[8px] font-bold tracking-[.38em] uppercase" style={{color:"rgba(255,255,255,.3)"}}>{sl.tag}</span>
                </div>
                <p className="hB hEnt d3 font-medium leading-relaxed mb-7" style={{fontSize:"clamp(.83rem,1vw,.97rem)",color:"rgba(255,255,255,.62)",maxWidth:"50ch"}}>{sl.intro.replace(/\\n/g," ")}</p>
                <div className="hEnt d4 flex items-center gap-3">
                  <a href="#contact" onClick={e=>scrollTo(e,"#contact")} className="bP hB inline-flex items-center gap-2.5 py-3.5 px-7 text-xs font-extrabold tracking-[.17em] uppercase" style={{background:sl.accent,color:"#fff",boxShadow:`0 0 48px ${sl.accent}38`}}>{sl.cta.primary}<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M2.5 6.5h8M7.5 3.5l3 3-3 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg></a>
                  <a href="#services" onClick={e=>scrollTo(e,"#services")} className="bG hB inline-flex items-center gap-2.5 py-3.5 px-7 text-xs font-bold tracking-[.17em] uppercase">{sl.cta.secondary}</a>
                </div>
              </div>
              <div className="hEnt d5 flex-shrink-0 text-right">
                <div className="sC inline-block p-5 relative overflow-hidden"
                  ...
                  <div className="hM text-[8px] font-bold tracking-[.3em] uppercase mt-1.5" style={{color:"rgba(255,255,255,.32)"}}>{sl.stat.label}</div>
                  <div className="mt-4 pt-3.5 flex items-center gap-2.5" style={{borderTop:`1px solid ${sl.accent}1e`}}>
                    ...
                    <span className="hM text-[8px] font-bold tracking-widest uppercase" style={{color:"rgba(255,255,255,.26)"}}>Enterprise Grade</span>
                  </div>
                </div>
              </div>
            </motion.div>
